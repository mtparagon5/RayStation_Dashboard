...
function fillDVHStatsTable(d) {
  ...
  $("#datatables-table").DataTable({
    ...
    columns: [
      ...
      { title: "Plan" },
      { title: "Global Max [Gy]" },
      { title: "Global Max %&#916" },
      { title: "Structure Max [Gy]" },
      { title: "Structure Max %&#916" },
      { title: "D95 [Gy]" },
      { title: "D95 %&#916" }
    ],
    paging: true,
...
// aggregates max dose data (targets and oars)
function getGlobalMaxAggData(planGlobalMaxData) {
  var globalMaxAggData = d3
    .nest()
    .key(function(d) {
      return d[0];
    })
    ...
    .entries(planGlobalMaxData);

  return [globalMaxAggData];
}
...
function fillAggDataTables(datalist) {
  // fill tables
  $("#global-max-agg-data-table").DataTable({
    data: datalist[0][0],
    columns: [
      // structure
      // d.key,
  ...
  $("#global-max-agg-data-table th").addClass("text-center font-weight-light");
  $("#global-max-agg-data-table td").addClass("text-center font-weight-light");

  $("#structure-max-agg-data-table").DataTable({
    data: datalist[0][1],
    columns: [
      // structure
      // d.key,
      ...
      { title: "Pert Type / Structure Instance" },
      { title: "Max [Max[Gy]]" },
      { title: "Min [Max[Gy]]" },
      { title: "Range [Max[Gy]]" },
      { title: "Avg [Max[Gy]]" },
  ...
  $("#structure-max-agg-data-table th").addClass(
    "text-center font-weight-light"
  );
  $("#structure-max-agg-data-table td").addClass(
    "text-center font-weight-light"
  );
...
$(document).on("click", "#check-boxes input", function(e) {
  ...
  $(e.target)
    .parent()
    .toggleClass("text-muted striked");
    ...
    if (series.userOptions.name.includes(e.target.id)) {
      ...
      if (!e.target.checked) {
        ...
        if (!toIgnore.includes(series.userOptions.linkedTo)) {
          ...
          // if linkedTo series not visible
          series.setVisible(false, false);
        }
      }
    }
